...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear((opt.n_classes + int(np.prod(img_shape))), 512), 
                                   ...
                                   nn.Dropout(0.4), 
                                   nn.LeakyReLU(0.2), 
                                   # DONE: 添加最后一个线性层，最终输出为一个实数
                                   nn.Linear(512, 1),
                                   )

    def execute(self, img, labels):
        d_in = jt.contrib.concat((img.view((img.shape[0], (- 1))), self.label_embedding(labels)), dim=1)
        # DONE: 将d_in输入到模型中并返回计算结果
...
for epoch in range(opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        d_real_loss = adversarial_loss(validity_real, valid) # DONE

        validity_fake = discriminator(gen_imgs.stop_grad(), gen_labels)
        d_fake_loss = adversarial_loss(validity_fake, fake) # DONE
...
number = '2019011354' #DONE: 写入你的学号（字符串类型）
n_row = len(number)
